...
<div class="page-container">
	<div class="text-l"> 关键字：
		<input type="text" class="input-text query-text" style="width:250px" placeholder="输入账号、姓名、手机号码" id="keyword" >
		<button type="button" class="btn btn-success" id="queryBtn"><i class="Hui-iconfont">&#xe665;</i> 查询</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a href="javascript:;" onclick="add('添加用户')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>添加用户</a>
			<a href="javascript:;" class="btn btn-secondary radius" id="userRole"><i class="Hui-iconfont">&#xe61d;</i>配置角色</a>
		</span> 
...
function query(curr){
	...
	ajaxPost("${base}/sysUser/query",{keyword:keyword,pageNumber:curr || 1,pageSize:pageSize},function(data){
		laypage({
            cont: 'page1', //容器。
		...
		$.each(data.list,function(id,item){
			...
			html +='<td>'+nullToSpace(item.real_name)+'</td>';
			...
			if(item.status=="0"){
				html +=	'<a style="text-decoration:none" onClick="updateStatus(this,\''+item.status+'\',\''+item.id+'\')" href="javascript:;" title="停用">';
				html +=		'<i class="Hui-iconfont">&#xe631;</i>';
				html +=	'</a>';
			}else{
				html +=	'<a style="text-decoration:none" onClick="updateStatus(this,\''+item.status+'\',\''+item.id+'\')" href="javascript:;" title="起用">';
				html +=		'<i class="Hui-iconfont">&#xe615;</i>';
				html +=	'</a>';
			}
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		ajaxPost("${base}/sysUser/delete",{id:id},function(data){
			if(data.success){
...
function updateStatus(obj,flag,id){
	...
	layer.confirm('确认要'+statusName+'吗？',function(index){
		ajaxPost("${base}/sysUser/updateStatus",{id:id,status:status},function(data){
			if(data.success){
				if(flag=="0"){
